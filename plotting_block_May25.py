    ...
    if ((current_time - last_time).total_seconds()) >= 1:
        ...
        if newestData:
            ...
            if warning_showed <=0.001:   # green color, normal driving
                plt.plot(0,0,'green',alpha=0.5,marker='o', markersize=40)[0]

            elif 0.001<warning_showed <=yellow_limit:  # yellow color, mild braking
                if warning_showed  <= yellow_limit/3:   # warning smaller than 10
                    plt.plot(0,0,'gold',alpha=0.5,marker='o', markersize=warning_showed *5+40)[0]
                else:   # warning larger than 10 but still yellow
                    plt.plot(0,0,'gold',alpha=0.5,marker='o', markersize=(warning_showed  - yellow_limit/3)*3 + (5 * yellow_limit/3) + 40)[0]
            else:   # warning larget than limit, red color, harsh braking
                plt.plot(0,0,'red',alpha=0.5,marker='o', markersize=(min( (warning_showed - yellow_limit )*3  +  (5*yellow_limit/3) + 40 , 330)) )[0]
        # Update the display
            plt.draw()
            plt.pause(0.01)
        else:
