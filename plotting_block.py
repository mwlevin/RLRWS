    ...
    if ((current_time - last_time).total_seconds()) >= 1:
        ...
        if newestData:
            ...
            if passer(pos_ego, spd_ego, predicted_tl_state):
                ...
                if warning_signal_ego[0]<=0.001:   # green color, normal driving
                    plt.plot(0,0,'green',alpha=0.5,marker='o', markersize=50)[0]

                elif 0.001<warning_signal_ego[0]<=yellow_limit:  # yellow color, mild braking
                    if warning_signal_ego[0] <= yellow_limit/3:   # warning smaller than 10
                        plt.plot(0,0,'gold',alpha=0.5,marker='o', markersize=warning_signal_ego[0]*10+50)[0]
                    else:   # warning larger than 10 but still yellow
                        plt.plot(0,0,'gold',alpha=0.5,marker='o', markersize=(warning_signal_ego[0] - yellow_limit/3)*4 + (10 * yellow_limit/3) + 50)[0]
                else:   # warning larget than limit, red color, harsh braking
                    plt.plot(0,0,'red',alpha=0.5,marker='o', markersize=(warning_signal_ego[0] - yellow_limit )*4  +  (yellow_limit*2/3 + 10 * yellow_limit/3 + 50) )[0]
